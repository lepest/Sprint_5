...
class TestFollowToPersonalAccount:
    def test_go_to_personal_account(self, driver):
        ...
        assert WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located(TestLocators.SEARCH_LIST_SAUSE))
        driver.find_element(*TestLocators.SEARCH_SECTION_SMALL_BREADS).click()
        assert WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located(TestLocators.SEARCH_LIST_SMALL_BREADS))
        driver.find_element(*TestLocators.SEARCH_SECTION_FILLING).click()
        assert WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located(TestLocators.SEARCH_LIST_FILLING))
        driver.quit()